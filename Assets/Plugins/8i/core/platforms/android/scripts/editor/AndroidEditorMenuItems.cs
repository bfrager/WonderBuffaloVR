...
namespace HVR.Android.Editor
{
    public class AndroidEditorMenuItems : MonoBehaviour
    {
        ...
        [MenuItem("8i/Android/Build and Run (Assets Packed)", false, 11)]
        static void Android_Build(MenuCommand menuCommand)
        {
            if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)
            {
                ...
                PostBuildDataCopier.instance.ExportAssetData(androidAssetsPath);

                BuildPipeline.BuildPlayer(buildScenes, buildPath, BuildTarget.Android, BuildOptions.AutoRunPlayer);

                PostBuildDataCopier.instance.CleanHRVAssetClipData(androidAssetsPath);
            }
            else
            {
        ...
        [MenuItem("8i/Android/Export HvrActor data for this build (Assets Loose)", false, 12)]
        static void Android_ExportHvrData(MenuCommand menuCommand)
        {
            ...
            PostBuildDataCopier.instance.CleanHRVAssetClipData(buildPath + "/");
            PostBuildDataCopier.instance.ExportAssetData(buildPath + "/");
